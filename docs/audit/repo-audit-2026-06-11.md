...
| # | Task | Files/areas | Acceptance criteria | Effort | Risk | Deps |
|---|------|-------------|--------------------|--------|------|------|
| 2.1 | **Extract MessageOrchestrator** from telegram.ts (policy confirm, learning session, auto-skills, healing); Telegram keeps transport/format only. *Owner decision 2026-06-11: Matrix/Web parity is an ideal, not a priority — the primary motivation is testability and containment of telegram.ts.* **DONE rc.114 (steps a–c):** all five pre-routing gates (auto-skill proposal, tool confirmation, skill healing, learning session, orchestration) extracted to `src/core/message-gates.ts` behind a platform-neutral `GateIO` adapter; characterization suite green unchanged. The original "<2,000 lines" target was miscalibrated — the bulk of telegram.ts is ~60 bot.command registrations (genuinely platform code), not orchestration; post-extraction size is ~4,250 lines with all conversation-flow logic now platform-neutral and unit-testable without grammy. | src/core/message-gates.ts, telegram.ts | characterization tests (0.4) green unchanged ✓; gates testable without grammy mocks ✓; Matrix wiring = deferred feature work | XL → (a)(b)(c) done; (d) see 2.2 note | Med–High | 0.1, 0.4 |
| 2.2 | Extract shared CSV ingestion service + shared markdown formatter | telegram.ts ~:3000–3400, matrix.ts:34–80 | Same CSV upload works on Matrix; one formatter, two option sets | M | Med | 2.1a–c not required, can go first. *rc.114 finding: premise partially stale — the CSV domain logic (executeFmeaFromCsv, executeInventoryAnalysis, executeCapabilityAnalysis, executeBalance, attendance ingestion) had already been extracted to domain modules in earlier RCs; what remained in telegram.ts was the duplicated Telegram file-download dance (now one `downloadDocumentText` helper) plus grammy-specific photo replies. Remaining for true Matrix parity: a photo-capable GateIO and Matrix-side caption commands — feature work, not refactor.* |
...
7. ~~**Domain-intent questions surfaced by the rc.113 dead-code sweep**~~ — **All five resolved with the owner (rc.115):** (a) capacity P25/P75 — **fixed**, quartiles added to the Monte Carlo histogram subtitle; (b) DOE `createConfirmationRun` — **fixed (was a real defect)**: prediction is now the additive main-effects model at the chosen factor levels (`ŷ = ȳ + Σ(mean@level − ȳ)`), not the grand mean; pinned with exact-value tests; (c) Gantt residual idle gaps — **fixed**: uncovered sub-intervals around partial setups now render as Idle bars (3 regression tests); (d) balance export source — **owner decision: keep recompute** (all actions recompute consistently; deterministic algorithm; stored snapshots would desync export from charts); (e) warmup test — **fixed**: `noWarmup.warmup_time_lost === 0` baseline added (a same-seed throughput comparison was evaluated and rejected — warmup draws shift the stochastic sample path).

**Bonus defects found and fixed during the rc.115 test-gap closeout:** FSM `sanitizeName` emitted illegal IEC 61131-3 identifiers for digit-leading state names (the `_` prefix was stripped right back off — fixed, pinned); `calculateStreak` undercounted by exactly one day whenever today had no session yet (loop-bound off-by-one — fixed, pinned). A behavioral wart was pinned-not-changed: a below-goal session today zeroes the streak while no session today is forgiven (defensible semantics; owner can revisit).
