...
- **[HIGH, FACT] The IPC layer has zero tests.** src/ipc/ (~686 lines: client.ts 394, server.ts 133, env-whitelist.ts) — spawn, heartbeat, restart-backoff, message routing all unexercised. This is the system's concurrency backbone; a hung pending-request or failed restart would only be discovered in production.
- **[HIGH, FACT] The web API/auth surface has zero direct tests.** src/web/ is ~4.8k lines (server.ts 1,010 + 11 dashboard API files); only WebSocket behavior is indirectly touched by platform-integration tests. Status codes, request validation, and token-auth integration are unverified — and this is the one network-exposed surface.
