...
- **NovaLink SAM**: full read-write access to the SAM analysis system (quoting/per-piece billing) via the `sam` pack (7 `sam_*` tools, tools process). Config: `NOVALINK_SAM_URL` + `NOVALINK_SAM_API_KEY` (.env, whitelisted to the tools process). Writes + `sam_generate` require SA4 confirmation; `sam_generate` carries a 960 s IPC budget (`ToolEntry.timeoutMs`). SAM's AI legs run on the subscription backend (`claude -p`): a 4-file `generate-mm` measured **~7 min to insert the analysis row and ~12 min to finish writing it** — and when the client gives up, **the server keeps going and stores the analysis anyway** (that is how duplicate drafts accumulated). So the wrapper bounds the connection and then recovers: curl 600 s, then up to 300 s polling for the stored result, returned with `_recovered_after_client_timeout`. Ladder: wrapper 900 s < bash/IPC 930–960 s < `CLAUDE_TIMEOUT_MS` 1200 s — raise them together or not at all; the top rung covers the WHOLE turn (read files → generate → compose), not just the slowest call. `generate`/`generate-mm` strip the 20-section `full_json` from what the model sees (same rule as `sam get`); a `persist:false` draft is exempt from both the strip and the recovery — nothing is stored, so there is no id to re-fetch. Note `created_at` is when SAM inserts the row, **not** when the draft is done; `updated_at` is. Handoff: `reference/novalink-sam-handoff.md` (v1.1). Phase-2 analytics (review/balance/cells/estimate/calc-library) are Claude-path only via the `sam` wrapper.
