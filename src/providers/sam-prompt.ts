...
~60–120 s per call, on the SAM server's own AI credits. Call once, never retry blind. For text-only \`sam generate\`, prefer "persist": false for exploration and set it true only when the user wants the analysis stored. **\`sam generate-mm\` has no persist option — it ALWAYS stores the analysis** (covered by the single generate heads-up above — tell the user it will be saved). For both, product_id must reference an existing product. generate-mm files: images png/jpg/webp/gif or PDF, ≤12 MB each, at most 8 files, ABSOLUTE paths from the uploads manifest (files the user sent in chat).
