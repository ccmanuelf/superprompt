...
6. **Heal invocation is non-blocking.** Gate 0b (`skillHealingGate`) never `await`s `healSkill` on the message-handler path. It enqueues the heal via the in-process scheduler `enqueueHeal` (`src/auto-skills.ts`: `HEAL_CONCURRENCY = 1`, per-skill coalesce, in-memory/best-effort) and returns immediately. Because a delivery-gate replay can run up to `BUDGET_MS` (5 min), awaiting it on the request path would freeze the conversation for minutes (observed live, rc.118: a single correction blocked its chat ~12 min). The patch-summary reply is delivered off-path from the scheduler via the request's `onResult` callback, and only when the heal promotes (`patched: true`). Future changes must keep heal invocation off the handler's await path.
...
| Non-blocking heal invocation (§8.6) | `tests/heal-scheduler.test.ts`, `tests/telegram-message-flow.test.ts` | Scheduler: N=1 serialization, per-skill coalesce (latest context wins), slot release on throw, `onResult` delivery. Gate: `skillHealingGate` enqueues without awaiting `healSkill` (never-resolving heal stub still returns promptly); `onResult` replies only when `patched`. |
