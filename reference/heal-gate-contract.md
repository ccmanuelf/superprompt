...
6. **Heal invocation is non-blocking.** No platform message-handler path ever `await`s `healSkill`. All four heal trigger sites — Telegram and Matrix, each with a correction-triggered heal (Gate 0b) and a low-quality-triggered heal — enqueue via the in-process scheduler `enqueueHeal` (`src/auto-skills.ts`: `HEAL_CONCURRENCY = 1`, per-skill coalesce, in-memory/best-effort) and return immediately. Because a delivery-gate replay can run up to `BUDGET_MS` (5 min), awaiting it on the request path would freeze the conversation for minutes (observed live, rc.118: a single correction blocked its chat ~12 min). The patch-summary reply is delivered off-path from the scheduler via the request's `onResult` callback, and only when the heal promotes (`patched: true`). Future changes must keep heal invocation off the handler's await path. Coalescing is keyed by `skillId`, not `chatId`: if the same skill is corrected from two chats while a heal is queued, the queued request's `chatId`/`onResult` is overwritten, so a superseded chat may not receive its "skill improved" notice. The heal still applies (the skill's `system_prompt` is shared), so this is acceptable degradation.
...
| Non-blocking heal invocation (§8.6) | `tests/heal-scheduler.test.ts`, `tests/telegram-message-flow.test.ts` | Scheduler: N=1 serialization, per-skill coalesce (latest context wins), slot release on throw, `onResult` delivery. Telegram Gate 0b: `skillHealingGate` enqueues without awaiting `healSkill` (never-resolving heal stub still returns promptly); `onResult` replies only when `patched`. The Matrix Gate 0b and both quality-triggered heals (Telegram + Matrix) route through the same `enqueueHeal` scheduler, so the scheduler tests cover their non-blocking behavior structurally. |
