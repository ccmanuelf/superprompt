...
**Mechanical-validation host:** dev Mac (this machine), model `qwen3.5:2b` (ministral-3:3b not pulled locally; per task instructions any pulled model is fine for Step 2 — it only proves the script runs)
...
3. Schema cost is real and linear-ish: 48 tools cost 5817 prompt tokens vs 3357 for 20 tools (~46 tokens/tool for this synthetic schema) — trimming the tool set directly cuts prompt-eval token count.
