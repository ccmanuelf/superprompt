...
| `ollama` | `^0.6.3` | Ollama JS SDK | Official SDK. Supports tool calling, streaming, chat API. |
| `openai` | `^6.36.0` | OpenAI-compatible client | Used for Speaches voice API (not for OpenAI itself). |
...
| `pdfkit` | `^0.18.0` | Write PDF | Programmatic PDF generation. |
...
| Package | Version | Purpose | Notes |
|---------|---------|---------|-------|
| `typescript` | `^6.0.3` | TypeScript compiler | ES2022 target, NodeNext module resolution. |
| `tsx` | `^4.21.0` | TypeScript executor | For running .ts files directly during development. |
| `@types/node` | `^25.6.0` | Node.js type definitions | TS-only types; ahead of the runtime engine pin (Node 20). |
| `@types/better-sqlite3` | `^7.6.13` | SQLite type definitions | |
| `vitest` | `^4.1.5` | Test framework | Fast, ESM-native. Module-mock factories require `vi.hoisted` (vitest-4 idiom). |
