#!/usr/bin/env bash
# clauded — Interactive Setup Script
...
echo ""
echo -e "${BOLD}╔══════════════════════════════════════════╗${NC}"
echo -e "${BOLD}║         clauded Setup Wizard             ║${NC}"
if $REPLICATE; then
...
if confirm "Build and start the clauded container now?"; then
  info "Building clauded container..."
  docker compose up -d --build clauded 2>&1 | tail -5

  # Wait a moment for startup
  sleep 3

  # Check if running
  if docker compose ps clauded 2>/dev/null | grep -q "running\|Up"; then
    success "clauded is running!"
    echo ""
    echo -e "${BOLD}Container logs (last 10 lines):${NC}"
    docker logs clauded-bot --tail 10 2>&1 || true
  else
    warn "Container may not have started correctly."
    echo "  Check logs with: docker logs clauded-bot"
  fi
else
  info "Skipping build. Run manually with:"
  echo "  docker compose up -d --build clauded"
fi
...
echo "  Start:   docker compose up -d clauded"
echo "  Stop:    docker compose down"
echo "  Rebuild: docker compose up -d --build clauded"
echo "  Logs:    docker logs -f clauded-bot"
echo ""
